...
class WC_CoolPay_API_Payment extends WC_CoolPay_API_Transaction {
	 ...
	 * capture function.
	 *
	 * Sends a 'capture' request to the CoolPay API
	 *
	 * @access public
	 *
	...
	public function capture( $transaction_id, $order, $amount = null ) {
		...
		return $this->post( sprintf( '%d/%s', $transaction_id, "capture" ), array( 'amount' => WC_CoolPay_Helper::price_multiply( $amount ) ) );
	}
	 ...
	 * refund function.
	 *
	 * Sends a 'refund' request to the CoolPay API
	 *
	 * @access public
	 *
	 * @param  int       $transaction_id
