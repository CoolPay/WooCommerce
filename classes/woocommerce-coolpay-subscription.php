<?php
/**
 * WC_CoolPay_Subscription class
 *
 * @class 		WC_CoolPay_Subscription
 * @version		1.0.0
...
class WC_CoolPay_Subscription {
	...
	public static function get_initial_subscription_transaction_id( $order ) {
		...
		if( $is_subscription ) {
			$original_order = new WC_CoolPay_Order( $order->post->post_parent );
			return $original_order->get_transaction_id();
		} 
		else if( self::is_renewal( $order ) ) {
			$subscriptions = self::get_parent_order( $order );
			$subscription = end( $subscriptions );
			$original_order = new WC_CoolPay_Order( $subscription->post->post_parent );
			return $original_order->get_transaction_id();
		}
    ...
    */
	 ...
	 *
	 * @access public static
	 * @param  [WC_CoolPay_Order] $order [description]
	 * @return boolean
	 */
	...
	*/
    ...
    /**
     * @param WC_CoolPay_Order $order The parent order
     * @return bool
     */
	public static function get_subscription_id(WC_CoolPay_Order $order) {
		$order_id = $order->get_id();
